...
<div class="p-5 w-full grid grid-cols-2 gap-5 justify-center items-center">
    <form action="{{ route('worker.update', $worker_data['id']) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input value="{{ $worker_data['id'] }}" type="text" id="rut" name="rut" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder="12.345.678-9" required>
        </div>
        <div class="flex flex-row gap-5 mb-6">
            <div class="w-full">
                <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Nombre</label>
                <input value="{{ $worker_data['name'] }}" type="text" id="name" name="name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder="Juan" required>
            </div>
            <div class="w-full">
                <label for="last_name" class="block mb-2 text-sm font-medium text-gray-900">Apellido</label>
                <input value="{{ $worker_data['last_name'] }}" type="text" id="last_name" name="last_name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder="Perez" required>
            </div>
        </div>
        <div class="mb-6">
            <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Email</label>
            <input value="{{ $worker_data['email'] }}" type="email" id="email" name="email" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder="example@email.com" required>
        </div>
        {{-- <div class="flex flex-row gap-5 mb-6">
            <div class="w-full">
